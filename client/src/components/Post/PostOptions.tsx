    ...
    return (
        <>
            ...
            {showOptions && showOptions !== "" && (
                <div
                    ref={wrapperRef}
                    className="search-card flex flex-col py-1 font-bold top-0 right-0 absolute z-50 bg-[color:var(--element-bg-color-elevate-1)] ring-1 ring-[color:var(--border-color)]"
                >
                    <button
                        onClick={(event: React.MouseEvent<HTMLElement>) => {
                            event.stopPropagation();
